...
- Keep workflow entrypoints explicit by passing the intended module to `ghawb render` with declared `--input` / `--output` pairs, or using the root `generate:workflows` command.
