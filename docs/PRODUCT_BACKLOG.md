...
- Implementation Plan: Add a build step to each package that emits ESM `.js` and `.d.ts` files into `dist/`. Update `package.json` exports to point to `dist/` for npm consumers while preserving source-oriented exports in `jsr.json` for JSR/Bun/Deno. Change CLI shebang from `#!/usr/bin/env bun` to `#!/usr/bin/env node`. Publish the CLI package under the `ghawb` name on npm (in addition to `@ghawb/cli`) so that `npx ghawb` resolves without scoped-package syntax, or add a `bin` entry in a root-level publishable wrapper — the concrete strategy must be decided during implementation. Add a `prepublishOnly` or `prepack` script that runs the build. Validate install-to-success from a clean external directory. Update CI to verify built artifacts.
- Definition of Done: All three packages emit built artifacts suitable for npm consumers. CLI is executable via `node`. A public npm command surface exists such that `npx ghawb` (or `npx @ghawb/cli`) resolves and runs. JSR source publishing remains functional. Build step integrated into CI. Install-to-success validated from a clean directory. SPEC.md distribution section updated. Code review completed.
- Acceptance Criteria: `npm pack` produces a tarball with `dist/` containing `.js` and `.d.ts` files. The CLI is invocable via `npx` without Bun installed (exact package name — `ghawb` or `@ghawb/cli` — is decided during implementation and documented). JSR publishing continues to use source exports. No runtime regression in Bun, Node, or Deno test suites.
- Story Points: 5
- Status: ready
- Completed At: N/A
...
- Prerequisites: Item 41 (release packaging) must land first so install instructions reference built artifacts rather than source-only paths.
