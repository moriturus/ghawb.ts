...
- Acceptance Criteria: `npm pack` produces a tarball with `dist/` containing `.js` and `.d.ts` files. `npx ghawb render --input <workflow.ts> --output <workflow.yml>` works without Bun installed. JSR publishing continues to use source exports. No runtime regression in Bun, Node, or Deno test suites.
- Story Points: 5
- Status: ready
- Completed At: N/A
