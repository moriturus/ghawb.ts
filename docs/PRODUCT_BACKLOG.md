...
- Sprint 22 discovery decision: Item 75a retained `nodeCi()` in `@ghawb/sdk` and rejected both a new helper package and reuse of an existing opt-in package for the current narrow helper. Because migration was not approved, Sprint 22 stopped before Item 75b and the active backlog resumes with Item 77. See [ADR 0005](./adrs/0005-retain-node-ci-helper-in-sdk.md) and [Sprint 22 Backlog](./sprint_backlogs/sp22.md).
