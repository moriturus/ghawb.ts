...
- Implementation Plan: Update PLAYBOOK.md and related sprint-operation rules to add explicit closeout waiting behavior rules (wait / handoff / follow-up decision tree when hosted proof is pending), add evidence provenance requirements to sprint review and retrospective notes (clean snapshot vs. scoped dirty-worktree), define clean-branch or scoped-verification gate criteria before sprint closeout, add external proof planning expectations to the planning and refinement protocol, make it explicit that sprint execution does not stop merely because a response turn or status report ended, and add a closeout checklist step that synchronizes sprint summary status plus sprint review / retrospective / docs index records. Update CONTRIBUTING.md if any contributor-facing verification expectations change.
- Definition of Done: PLAYBOOK.md and the relevant sprint-operation rules document explicit closeout waiting behavior, evidence provenance requirements, verification gate criteria, external proof planning expectations, no-false-stop execution rules, and sprint-document synchronization expectations. The six originating BOARD items are closed or marked concretized into this item. CONTRIBUTING.md is updated if applicable. The change is reviewed by a non-implementing persona.
- Acceptance Criteria: PLAYBOOK.md contains a concrete decision tree for closeout waiting behavior when hosted proof is pending, sprint review and retrospective templates require an explicit evidence provenance note, the closeout protocol includes a verification-target decision gate (clean branch or scoped file set), the planning protocol includes an external proof requirement step, sprint-execution rules state that response-turn boundaries are not stop conditions, and the closeout checklist requires sprint backlog summary plus review / retrospective / docs index synchronization before ceremony completion.
- Story Points: 2
- Status: pending
- Completed At: N/A
