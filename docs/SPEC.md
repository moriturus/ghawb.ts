...
- The SDK is published as six packages: `@ghawb/sdk` (core workflow builder and renderer), `@ghawb/shared` (shared types, identifiers, and errors), `@ghawb/composite-actions` (opt-in composite action authoring and rendering), `@ghawb/typed-actions` (opt-in typed wrappers for common actions), `@ghawb/cli` (YAML generation CLI), and `@ghawb/yaml-import` (opt-in reusable-workflow YAML import).
...
- Package entry points: `@ghawb/sdk`, `@ghawb/shared`, `@ghawb/composite-actions`, `@ghawb/typed-actions`, and `@ghawb/yaml-import` export from `./src/index.ts` (source) and `./dist/index.js` (built). `@ghawb/cli` exports from `./src/index.ts` and `./dist/index.js` and provides a `ghawb` binary entry at `./dist/bin.js` with a `#!/usr/bin/env node` shebang.
