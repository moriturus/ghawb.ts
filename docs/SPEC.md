...
- `@ghawb/sdk` currently supports a minimal builder-centered workflow model with:
  ...
  - discriminated job variants: step-based jobs with `runs-on` in string or string-array form plus inline `steps`, and reusable-workflow jobs with job-level `uses` typed as `WorkflowRef`, optional `with`, and `secrets` as either an explicit map or the shorthand `inherit`
