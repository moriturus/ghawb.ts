...
- `@ghawb/sdk` currently supports a minimal builder-centered workflow model with:
  ...
  - a narrow additive Node CI helper via `nodeCi(job, options)` in opt-in package `@ghawb/job-helpers`, which appends explicit `checkout`, `setup-node`, `install`, and `test` steps to a job builder for the common single-job Node CI path; accepts `NodeCiOptions` with `nodeVersion` (required), `install?`, `test?`, `cache?`, `cacheDependencyPath?`; returns the job builder for optional chaining
