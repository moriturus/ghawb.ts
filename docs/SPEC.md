...
- The repository is organized as a workspace with `@ghawb/sdk`, `@ghawb/shared`, `@ghawb/job-helpers`, `@ghawb/composite-actions`, `@ghawb/typed-actions`, `@ghawb/cli`, and `@ghawb/reusable-workflow-import`.
- Root developer commands provide linting, formatting, type-checking, and test entrypoints across Bun and Deno.
...
- a generic `JobBuilder.apply(helper)` hook for opt-in job helpers, plus narrow additive Node CI/bootstrap helpers in `@ghawb/job-helpers`; the preferred builder-style path is `job.apply(nodeCi({ nodeVersion: "24" }))` for the full CI path or `job.apply(nodeBootstrap({ nodeVersion: "24" }))` for the bootstrap prefix, while legacy `nodeCi(job, options)` remains supported for migration; `nodeCi` appends explicit `checkout`, `setup-node`, `install`, and `test` steps to a job builder for the common single-job Node CI path and accepts `NodeCiOptions` with `nodeVersion` (required), `install?`, `test?`, `cache?`, `cacheDependencyPath?`; `nodeBootstrap` appends explicit `checkout`, `setup-node`, and `install` steps and accepts `NodeBootstrapOptions` with `nodeVersion` (required), `install?`, `cache?`, `cacheDependencyPath?`, and `registryUrl?`
  ...
  - step metadata fields `name`, `env`, `with`, `if`, `continue-on-error` (boolean), and `timeout-minutes` (positive integer); the metadata parameter of `uses()`, `run()`, and `runScript()` also accepts a plain `string` as shorthand for `{ name: value }`, e.g., `job.uses("actions/checkout@v6", "Checkout")` is equivalent to `job.uses("actions/checkout@v6", { name: "Checkout" })`; `uses()` also accepts typed action-step objects as its first argument, and when one is used the `with` inputs must come from the typed action object rather than `metadata.with`
...
- Bun-run Vitest remains the primary repository test authority, and a shared Deno compatibility suite exercises representative supported workflow fixtures across Bun and Deno.
- Deno remains intentionally scoped to compatibility-oriented coverage rather than the full repository test surface; the Deno suite combines the shared render conformance fixtures with representative public-entrypoint and authoring-flow checks for supported packages, including `@ghawb/reusable-workflow-import`.
...
- The contributor-facing local verification path is `bun run verify:pre-push`, which checks for a clean worktree, runs `bun run verify:workflows`, runs the root Bun checks (including `bun run verify:docs`), and runs `bun run coverage` before push.
...
- Keep dependencies compatible with the supported Bun and Deno runtime targets.
- Implement workflow AST construction with TypeScript language features and runtime-portable dependencies.
...
- Preserve Deno compatibility.
- Use Bun as the default development runtime and JSR as the only package distribution channel.
...
- An opt-in `@ghawb/reusable-workflow-import` package imports existing reusable workflow YAML files without adding YAML input to the core SDK. It reads a workflow YAML file from a file-system path, validates that it contains a `workflow_call` trigger, and returns a `WorkflowRef` suitable for `usesWorkflow()`. The package depends on a YAML parser internally but does not re-export it. Under Deno, the import path hardens around the parser's debug-only `process.env` probes so representative import flows do not require `--allow-env`. Invalid YAML, missing `workflow_call` triggers, and non-object YAML content produce clear `ReusableWorkflowImportError` diagnostics.
