...
- The repository is organized as a workspace with `@ghawb/sdk`, `@ghawb/shared`, `@ghawb/typed-actions`, `@ghawb/cli`, and `@ghawb/yaml-import`.
...
- The SDK is published as five packages: `@ghawb/sdk` (core workflow builder and renderer), `@ghawb/shared` (shared types, identifiers, and errors), `@ghawb/typed-actions` (opt-in typed wrappers for common actions), `@ghawb/cli` (YAML generation CLI), and `@ghawb/yaml-import` (opt-in reusable-workflow YAML import).
