...
- `@ghawb/sdk` currently supports a minimal builder-centered workflow model with:
  ...
  - a narrow additive Node CI helper on `JobBuilder` via `job.nodeCi({ nodeVersion, install?, test?, cache?, cacheDependencyPath? })`, which appends `checkout`, `setup-node`, `install`, and `test` steps for the common single-job Node CI path while preserving the existing builder API for any extra customization before or after the helper call
