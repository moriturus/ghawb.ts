...
1. Require sprint review and retrospective notes to reference either a clean review snapshot or an explicitly scoped working-tree evidence note.
...
2. Add an explicit clean-branch or scoped-verification gate to sprint closeout.
...
3. Add an explicit Definition-of-Done evidence map to every sprint closeout.
...
5. Add an explicit pre-review consistency check to every sprint closeout.
...
6. Make delegated role ownership explicit at sprint start when multi-agent collaboration is expected.
...
9. Align team expectations for tooling decisions and delegation at sprint start.
