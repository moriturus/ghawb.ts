...
2. Standardize sprint item and closeout branches on `origin/main` rather than assuming local `main` is current.
...
3. Require sprint review and retrospective notes to reference either a clean review snapshot or an explicitly scoped working-tree evidence note.
...
4. Add an explicit clean-branch or scoped-verification gate to sprint closeout.
...
5. Add an explicit Definition-of-Done evidence map to every sprint closeout.
...
6. Tighten sprint-planning acceptance criteria for external proof before execution starts.
...
7. Add an explicit pre-review consistency check to every sprint closeout.
...
8. Make delegated role ownership explicit at sprint start when multi-agent collaboration is expected.
...
11. Align team expectations for tooling decisions and delegation at sprint start.
