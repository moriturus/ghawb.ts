# Sprint Backlog: Sprint 26

This document records the selected sprint backlog and planning decisions for Sprint 26.

## Sprint Summary

Capacity: 20 story points.

Selected implementation units for Sprint 26: 10/20 story points.

Status: ready — The sprint backlog is committed and ready for execution.

## Planning Notes

- Capacity decision: Sprint 26 keeps the standing 20 SP capacity and commits 10 SP, leaving 10 SP residual. The residual capacity is intentional because the active backlog is exhausted in strict order and should not be padded with speculative work.
- Selection decision: Sprint 26 commits Items 87, 88, 89, and 90 for a total of 10 SP.
- Ordering decision: Sprint 26 preserves strict backlog execution order: `Item 87` -> `Item 88` -> `Item 89` -> `Item 90`.
- Dependency decision: Item 88 remains dependent on Item 87 because the docs guardrail should codify the maintained contract only after the contract wording is reconciled. Items 89 and 90 remain later in order as independent follow-ups after the contract hardening work.
- Collaboration decision: Sprint execution remains single-item-at-a-time in committed order. Developer ownership should follow the implementation concern of each item, with docs and specification alignment in Item 87, guardrail automation in Item 88, helper ergonomics in Item 89, and onboarding guidance in Item 90. Planned review ownership should stay with a different persona than the implementation owner for each change.
- Verification decision: Item 87 should be verified by docs consistency checks against SPEC and the sprint record. Item 88 should be verified by focused automated checks that detect stale CLI flag wording across maintained docs. Item 89 should be verified by tests covering the helper's expected authoring behavior and by review against the package boundary. Item 90 should be verified by docs review for clarity and consistency across the package-adoption path.

## Committed Items

### Item 87: Reconcile CLI config contract across maintained docs

- Why: The shipped `render` contract now uses `--bulk` for render-plan manifests, per-target `--config` for injected render-time config, and `getRenderConfig<T>()` inside workflow modules. The maintained docs and sprint records should tell the same story so users do not have to reconcile stale wording themselves.
- Prerequisites: None.
- Implementation Plan: Update README, SPEC, and any affected sprint record wording so the canonical CLI contract is described once and consistently.
- Definition of Done: The maintained docs no longer contain the retired manifest-centric `--config` wording, and the shipped CLI contract is described consistently across repository-facing documentation.
- Acceptance Criteria:
  - README examples use `--bulk` and per-target `--config` only.
  - `getRenderConfig<T>()` is documented as the workflow-module access point for injected config.
  - Sprint records that mention the CLI config contract match the shipped behavior.
- Story Points: 2
- Status: done
- Completed At: 2026-04-06T05:12:56Z
- Notes/Links: Intake created from Sprint 25 review and retrospective follow-up. The item narrows the maintained CLI contract wording without expanding CLI behavior. Verification: `bun run verify:docs`, `bun test tests/node/docs-guardrails.test.ts`. Review: no findings from the non-implementing review persona. See [Sprint 25 Review](../sprint_reviews/sp25.md), [Sprint 25 Retrospective](../sprint_retrospectives/sp25.md), [README](../../README.md), and [SPEC](../SPEC.md).

### Item 88: Strengthen docs guardrails against stale CLI flag wording

- Why: `bun run verify:docs` passed even though one maintained README section still referenced the retired manifest spelling. The docs guardrail should catch this class of drift before it reaches `main` again.
- Prerequisites: Item 87 or the same docs cleanup effort.
- Implementation Plan: Expand the docs verification checks so they cover cross-section consistency and retired flag detection, not just a small canonical snippet set.
- Definition of Done: Renamed CLI flags and contract wording are checked by a guardrail that fails when stale or duplicated guidance reappears.
- Acceptance Criteria:
  - A stale `--config` manifest reference causes `verify:docs` to fail.
  - The guardrail covers the maintained CLI usage sections that users actually read.
  - The check remains deterministic and easy to run locally.
- Story Points: 3
- Status: done
- Completed At: 2026-04-06T05:15:31Z
- Notes/Links: Intake created from Sprint 25 retrospective follow-up. This item depends on the doc-sync work in Item 87 so the new guardrail codifies the corrected contract. Verification: `bun test tests/node/docs-guardrails.test.ts`, `bun run verify:docs`. Review: no findings from the non-implementing review persona. See [Sprint 25 Retrospective](../sprint_retrospectives/sp25.md) and [LEARN](../LEARN.md).

### Item 89: Add a narrow opt-in helper for a common workflow authoring pattern

- Why: The existing `job-helpers` package has already proven that a narrow helper can reduce repeated authoring noise without widening `@ghawb/sdk`. A second small helper slice can improve day-to-day workflow ergonomics for a common repetitive pattern.
- Prerequisites: Preserve the current package boundary and builder-style helper contract.
- Implementation Plan: Identify one high-frequency job sequence that can be expressed as a small opt-in helper, implement it in `@ghawb/job-helpers`, and keep the helper additive.
- Definition of Done: The helper shortens a repetitive workflow authoring path without introducing helper-specific methods into the core SDK.
- Acceptance Criteria:
  - The helper is opt-in and lives outside `@ghawb/sdk`.
  - Existing validation and render behavior remain unchanged.
  - A representative workflow becomes materially shorter or clearer when using the helper.
- Story Points: 3
- Status: done
- Completed At: 2026-04-06T05:20:02Z
- Notes/Links: Intake created from product discovery after Sprint 25 closeout. The item is intentionally small and should stay additive to the existing package boundary. Verification: `bun test packages/job-helpers/src/node-ci.test.ts packages/job-helpers/src/node-bootstrap.test.ts`, `bun run verify:workflows`, `bun run verify:docs`, and `bun run check`. Review: the non-implementing review persona raised one low-severity docs wording issue in API Reference, and it was corrected before completion. See [SPEC](../SPEC.md) and [Sprint 23 Review](../sprint_reviews/sp23.md).

### Item 90: Clarify the quickest package adoption path for new users

- Why: As the repository adds more opt-in packages, the adoption path becomes harder to infer from a quick README skim. A clearer “start here, then add this” path reduces initial friction.
- Prerequisites: None.
- Implementation Plan: Tighten the README package-selection guidance so the first installation and next-step recommendations are easier to follow.
- Definition of Done: The README clearly explains the shortest path for a user who starts from zero and wants to author workflows with the least confusion.
- Acceptance Criteria:
  - The README presents a clear order for adopting `@ghawb/sdk`, `@ghawb/job-helpers`, `@ghawb/typed-actions`, and `@ghawb/cli`.
  - The guidance explains when to add each optional package.
  - The wording is short enough to scan quickly.
- Story Points: 2
- Status: done
- Completed At: 2026-04-06T05:21:08Z
- Notes/Links: Intake created from product discovery after Sprint 25 closeout. This item is a documentation and onboarding clarity slice, not a package-boundary change. Verification: `bun run verify:docs`. Review: no findings from the non-implementing review persona. See [README](../../README.md) and [SPEC](../SPEC.md).
