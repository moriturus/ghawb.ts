...
describe("typedActionStep", () => {
  it("omits with when no inputs are provided", () => {
    expect(typedActionStep("actions/checkout@v4")).toEqual({ uses: "actions/checkout@v4" });
  });
  ...
  it("preserves explicit with inputs", () => {
    const action = typedActionStep("actions/setup-node@v4", {
      "node-version": "22",
      cache: "pnpm",
    });
    ...
    const workflow = defineWorkflow({
      ...
      .addJob(createJobId("build"), (job) =>
        job
          ...
          .uses(typedActionStep("actions/checkout@v4", { "fetch-depth": "1" }), "Checkout")
          .uses(
            typedActionStep("actions/setup-node@v4", {
              "node-version": "22",
              cache: "pnpm",
    ...
    expect(() =>
      workflow
        .addJob(createJobId("build"), (job) =>
          job
            .runsOn("ubuntu-latest")
            .uses(typedActionStep("actions/checkout@v4", { "fetch-depth": "1" }), {
              with: {
                ref: "refs/heads/main",
              },
            } as unknown as never)
