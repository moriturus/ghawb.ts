...
import { expr, needs } from "./expressions.js";
...
describe("workflow builder", () => {
    ...
    it("builds a uses step with an id", () => {
      const workflow = defineWorkflow({
        ...
        .addJob(createJobId("checkout"), (job) => {
          job.runsOn("ubuntu-latest").uses("actions/checkout@v6", { id: "checkout-step" });
        })
        .build();
  ...
  describe("actionRef factory", () => {
    it("accepts a valid external action reference", () => {
      expect(actionRef("actions/checkout@v6")).toBe("actions/checkout@v6");
    });
  ...
  describe("workflowRef factory", () => {
    ...
    it("rejects an action reference format", () => {
      expect(() => workflowRef("actions/checkout@v6")).toThrow(Error);
    });
  ...
  describe("build validation for uses references", () => {
    ...
    it("rejects a reusable workflow job with an invalid uses format", () => {
      const builder = defineWorkflow({
        ...
        .onPush()
        .addJob(createJobId("deploy"), (job) => {
          job.usesWorkflow("actions/checkout@v6" as unknown as WorkflowRef);
        });
  ...
  describe("usesWorkflow object injection", () => {
    it("accepts a WorkflowBuilder and derives local workflow ref", () => {
      ...
      expect(reusableJob.outputNames).toEqual(["artifact_url"]);
    });
    ...
    it("accepts explicit output names for external WorkflowRef strings", () => {
      const result = defineWorkflow({
        id: createWorkflowId("caller_explicit_outputs"),
        name: "Caller Explicit Outputs",
      })
        .onPush()
        .addJob(createJobId("deploy"), (job) => {
        ...
        .addJob(createJobId("notify"), (job) => {
          job
            .runsOn("ubuntu-latest")
            .needs(createJobId("deploy"))
            .run(`echo ${expr(needs("deploy").outputs("artifact_url"))}`);
        })
        .build();
    ...
    it("uses() accepts a string shorthand equivalent to { name: value }", () => {
      const shorthand = defineWorkflow({
        ...
        .addJob(createJobId("j"), (job) => {
          job.runsOn("ubuntu-latest").uses("actions/checkout@v6", "Checkout");
        })
        .build();
      ...
      const expanded = defineWorkflow({
        ...
        .onPush()
        .addJob(createJobId("j"), (job) => {
          job.runsOn("ubuntu-latest").uses("actions/checkout@v6", { name: "Checkout" });
        })
        .build();
