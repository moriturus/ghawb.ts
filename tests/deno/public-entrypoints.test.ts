...
Deno.test("Deno supports representative workflow authoring through public package entrypoints", () => {
  ...
  assert(checkJob.steps[0]?.uses === "actions/checkout@v6", "expected checkout wrapper to render");
  assert(
    checkJob.steps[1]?.uses === "actions/setup-node@v6",
...
Deno.test("Deno supports composite action rendering through the public package entrypoint", () => {
  ...
  assert(checkoutStep.uses === "actions/checkout@v6", "expected typed uses payload");
});

Deno.test({
  name: "Deno imports reusable workflows through the reusable-workflow-import public entrypoint without env permission",
  permissions: {
    env: false,
  ...
  async fn() {
    const ref = await importReusableWorkflow(
      `${Deno.cwd()}/tests/deno/fixtures/reusable-workflow-import-shared-build.yml`
    );
    assert(
      ref === "./.github/workflows/reusable-workflow-import-shared-build.yml",
